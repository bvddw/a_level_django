...
from postsapp.forms import LoginUser, RegistrateUser, SetUserData, SetNewPassword, Reactivate
...
def set_password(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.update_data(request.user.username)

            url = reverse('user:password_changed')
            return HttpResponseRedirect(url)
    else:
        form = SetNewPassword(initial={'username': request.user.username})
